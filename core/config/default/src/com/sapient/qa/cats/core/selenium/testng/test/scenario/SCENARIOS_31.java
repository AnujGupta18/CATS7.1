...
*
**/
public class  SCENARIOS_31  extends TestConfig{
	SoftAssert softAssert;
	String testDataKey;
	String sheetName;

	private static CATSLogger catsLogger = CATSLogger.getInstance(SCENARIOS_31.class);
    ...
    public synchronized void endScenario(ITestContext context) {
		synchronized (this) {
		    try {
				...
				String scenarioID = "SCENARIOS_31";
	...
	@Test (priority=1,groups={"SCENARIOS_31"})
	public void COMMON_LaunchSite1(ITestContext context) {
		...
		try {
			...
			testCase=new COMMON_LaunchSite(context,1,"SCENARIOS_31",0, this.getSuiteDriver());
	...
	@Test (priority=2,groups={"SCENARIOS_31"})
	public void COMMON_CLOSEOVERLAY2(ITestContext context) {
		COMMON_CLOSEOVERLAY testCase=null;
		try {
			LangDataSingleton.getInstanceOf("_ENV_"+this.suiteName).put("CURR_DATAKEY","");
			testCase=new COMMON_CLOSEOVERLAY(context,2,"SCENARIOS_31",0, this.getSuiteDriver());
			testCase.setTestDataKey("");
			testCase.executeTestcase();
	...
	@Test (priority=3,groups={"SCENARIOS_31"})
	public void HEADER_VerifyHeaders3(ITestContext context) {
		HEADER_VerifyHeaders testCase=null;
		try {
			LangDataSingleton.getInstanceOf("_ENV_"+this.suiteName).put("CURR_DATAKEY","");
			testCase=new HEADER_VerifyHeaders(context,3,"SCENARIOS_31",0, this.getSuiteDriver());
			testCase.setTestDataKey("");
			testCase.executeTestcase();
	...
	@Test (priority=4,groups={"SCENARIOS_31"})
	public void COMMON_CloseWindow4(ITestContext context) {
		COMMON_CloseWindow testCase=null;
		try {
			LangDataSingleton.getInstanceOf("_ENV_"+this.suiteName).put("CURR_DATAKEY","");
			testCase=new COMMON_CloseWindow(context,4,"SCENARIOS_31",0, this.getSuiteDriver());
			testCase.setTestDataKey("");
			testCase.executeTestcase();
