...
*
**/
public class  SCENARIOS_34  extends TestConfig{
	SoftAssert softAssert;
	String testDataKey;
	String sheetName;

	private static CATSLogger catsLogger = CATSLogger.getInstance(SCENARIOS_34.class);
    ...
    public synchronized void endScenario(ITestContext context) {
		synchronized (this) {
		    try {
				...
				String scenarioID = "SCENARIOS_34";
	...
	@Test (priority=1,groups={"SCENARIOS_34"})
	public void COMMON_LaunchSite1(ITestContext context) {
		...
		try {
			...
			testCase=new COMMON_LaunchSite(context,1,"SCENARIOS_34",0, this.getSuiteDriver());
	...
	@Test (priority=2,groups={"SCENARIOS_34"})
	public void COMMON_CLOSEOVERLAY2(ITestContext context) {
		COMMON_CLOSEOVERLAY testCase=null;
		try {
			LangDataSingleton.getInstanceOf("_ENV_"+this.suiteName).put("CURR_DATAKEY","");
			testCase=new COMMON_CLOSEOVERLAY(context,2,"SCENARIOS_34",0, this.getSuiteDriver());
			testCase.setTestDataKey("");
			testCase.executeTestcase();
	...
	@Test (priority=3,groups={"SCENARIOS_34"})
	public void HEADER_ClickCoffeeLink3(ITestContext context) {
		HEADER_ClickCoffeeLink testCase=null;
		try {
			LangDataSingleton.getInstanceOf("_ENV_"+this.suiteName).put("CURR_DATAKEY","");
			testCase=new HEADER_ClickCoffeeLink(context,3,"SCENARIOS_34",0, this.getSuiteDriver());
			testCase.setTestDataKey("");
			testCase.executeTestcase();
	...
	@Test (priority=4,groups={"SCENARIOS_34"})
	public void PLP_AddToCartt4(ITestContext context) {
		PLP_AddToCartt testCase=null;
		try {
			LangDataSingleton.getInstanceOf("_ENV_"+this.suiteName).put("CURR_DATAKEY","");
			testCase=new PLP_AddToCartt(context,4,"SCENARIOS_34",0, this.getSuiteDriver());
			testCase.setTestDataKey("");
			testCase.executeTestcase();
	...
	@Test (priority=5,groups={"SCENARIOS_34"})
	public void COMMON_CloseWindow5(ITestContext context) {
		COMMON_CloseWindow testCase=null;
		try {
			LangDataSingleton.getInstanceOf("_ENV_"+this.suiteName).put("CURR_DATAKEY","");
			testCase=new COMMON_CloseWindow(context,5,"SCENARIOS_34",0, this.getSuiteDriver());
			testCase.setTestDataKey("");
			testCase.executeTestcase();
