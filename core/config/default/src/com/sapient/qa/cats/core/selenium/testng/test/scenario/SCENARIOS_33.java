...
*
**/
public class  SCENARIOS_33  extends TestConfig{
	SoftAssert softAssert;
	String testDataKey;
	String sheetName;

	private static CATSLogger catsLogger = CATSLogger.getInstance(SCENARIOS_33.class);
    ...
    public synchronized void endScenario(ITestContext context) {
		synchronized (this) {
		    try {
				...
				String scenarioID = "SCENARIOS_33";
	...
	@Test (priority=1,groups={"SCENARIOS_33"})
	public void COMMON_LaunchSite1(ITestContext context) {
		...
		try {
			...
			testCase=new COMMON_LaunchSite(context,1,"SCENARIOS_33",0, this.getSuiteDriver());
	...
	@Test (priority=2,groups={"SCENARIOS_33"})
	public void COMMON_CLOSEOVERLAY2(ITestContext context) {
		COMMON_CLOSEOVERLAY testCase=null;
		try {
			LangDataSingleton.getInstanceOf("_ENV_"+this.suiteName).put("CURR_DATAKEY","");
			testCase=new COMMON_CLOSEOVERLAY(context,2,"SCENARIOS_33",0, this.getSuiteDriver());
			testCase.setTestDataKey("");
			testCase.executeTestcase();
	...
	@Test (priority=3,groups={"SCENARIOS_33"})
	public void COMMON_VerifyMenuBar3(ITestContext context) {
		COMMON_VerifyMenuBar testCase=null;
		try {
			LangDataSingleton.getInstanceOf("_ENV_"+this.suiteName).put("CURR_DATAKEY","");
			testCase=new COMMON_VerifyMenuBar(context,3,"SCENARIOS_33",0, this.getSuiteDriver());
			testCase.setTestDataKey("");
			testCase.executeTestcase();
	...
	@Test (priority=4,groups={"SCENARIOS_33"})
	public void COMMON_CloseWindow4(ITestContext context) {
		COMMON_CloseWindow testCase=null;
		try {
			LangDataSingleton.getInstanceOf("_ENV_"+this.suiteName).put("CURR_DATAKEY","");
			testCase=new COMMON_CloseWindow(context,4,"SCENARIOS_33",0, this.getSuiteDriver());
			testCase.setTestDataKey("");
			testCase.executeTestcase();
