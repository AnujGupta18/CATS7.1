...
*
**/
public class  SCENARIOS_35  extends TestConfig{
	SoftAssert softAssert;
	String testDataKey;
	String sheetName;

	private static CATSLogger catsLogger = CATSLogger.getInstance(SCENARIOS_35.class);
    ...
    public synchronized void endScenario(ITestContext context) {
		synchronized (this) {
		    try {
				...
				String scenarioID = "SCENARIOS_35";
	...
	@Test (priority=1,groups={"SCENARIOS_35"})
	public void COMMON_LaunchSite1(ITestContext context) {
		...
		try {
			...
			testCase=new COMMON_LaunchSite(context,1,"SCENARIOS_35",0, this.getSuiteDriver());
	...
	@Test (priority=2,groups={"SCENARIOS_35"})
	public void COMMON_CLOSEOVERLAY2(ITestContext context) {
		COMMON_CLOSEOVERLAY testCase=null;
		try {
			LangDataSingleton.getInstanceOf("_ENV_"+this.suiteName).put("CURR_DATAKEY","");
			testCase=new COMMON_CLOSEOVERLAY(context,2,"SCENARIOS_35",0, this.getSuiteDriver());
			testCase.setTestDataKey("");
			testCase.executeTestcase();
	...
	@Test (priority=3,groups={"SCENARIOS_35"})
	public void HEADER_ClickCoffeeMaker33(ITestContext context) {
		HEADER_ClickCoffeeMaker3 testCase=null;
		try {
			LangDataSingleton.getInstanceOf("_ENV_"+this.suiteName).put("CURR_DATAKEY","");
			testCase=new HEADER_ClickCoffeeMaker3(context,3,"SCENARIOS_35",0, this.getSuiteDriver());
			testCase.setTestDataKey("");
			testCase.executeTestcase();
	...
	@Test (priority=4,groups={"SCENARIOS_35"})
	public void PLP_BrewerAddToCart4(ITestContext context) {
		PLP_BrewerAddToCart testCase=null;
		try {
			LangDataSingleton.getInstanceOf("_ENV_"+this.suiteName).put("CURR_DATAKEY","");
			testCase=new PLP_BrewerAddToCart(context,4,"SCENARIOS_35",0, this.getSuiteDriver());
			testCase.setTestDataKey("");
			testCase.executeTestcase();
	...
	@Test (priority=5,groups={"SCENARIOS_35"})
	public void PDP_VerifyBrewerCompareTable5(ITestContext context) {
		PDP_VerifyBrewerCompareTable testCase=null;
		try {
			LangDataSingleton.getInstanceOf("_ENV_"+this.suiteName).put("CURR_DATAKEY","");
			testCase=new PDP_VerifyBrewerCompareTable(context,5,"SCENARIOS_35",0, this.getSuiteDriver());
			testCase.setTestDataKey("");
			testCase.executeTestcase();
	...
	@Test (priority=6,groups={"SCENARIOS_35"})
	public void COMMON_CloseWindow6(ITestContext context) {
		COMMON_CloseWindow testCase=null;
		try {
			LangDataSingleton.getInstanceOf("_ENV_"+this.suiteName).put("CURR_DATAKEY","");
			testCase=new COMMON_CloseWindow(context,6,"SCENARIOS_35",0, this.getSuiteDriver());
			testCase.setTestDataKey("");
			testCase.executeTestcase();
