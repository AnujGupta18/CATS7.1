...
*
**/
public class  BIRTHDAYGIFTVERIFYINGBIRTHDAYSECTIONDISAPPEAR extends DynamicInstTestcaseClassExtend {
	    
    public BIRTHDAYGIFTVERIFYINGBIRTHDAYSECTIONDISAPPEAR(ITestContext context,long scn_exe_tc_seq,String scenarioId,long scn_data_key_seq,RemoteWebDriver driver){
		    Properties prop;
			try {
	     	...
	     	LangDataSingleton.getInstanceOf("_ENV_"+this.suiteName).put("CURR_TCNAME","BIRTHDAYGIFTVERIFYINGBIRTHDAYSECTIONDISAPPEAR");
	     	this.scn_exe_tc_seq=scn_exe_tc_seq;
	     	this.testCaseId = "BIRTHDAYGIFT_VERIFYINGBIRTHDAYSECTIONDISAPPEAR";		
	}
		
	    
		
	public  synchronized void  executeTestcase() {
		catsLogger.console("Test case is running : BIRTHDAYGIFTVERIFYINGBIRTHDAYSECTIONDISAPPEAR");
		String locString="";
		try{
		...
		executeTestStep("217",  "VerifyElementNotPresent",": Verify the <SectioinBirthday> is Not Present",	suiteName, "BIRTHDAYGIFTVERIFYINGBIRTHDAYSECTIONDISAPPEAR", 
			actualData,expectedResult,actualDataRef,expectedResultRef,"",	"", "XPATH#del#//*[@class='row birthday-compaign']$del$",orRefName,"","");
		arg.updateSkippedTestStepsForTestcase("BIRTHDAYGIFT_VERIFYINGBIRTHDAYSECTIONDISAPPEAR",executedStepIds,this.exnInstId);	
	}
