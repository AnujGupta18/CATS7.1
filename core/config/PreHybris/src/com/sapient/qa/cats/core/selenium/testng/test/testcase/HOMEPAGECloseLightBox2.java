...
*
**/
public class  HOMEPAGECloseLightBox2 extends DynamicInstTestcaseClassExtend {
	    
    public HOMEPAGECloseLightBox2(ITestContext context,long scn_exe_tc_seq,String scenarioId,long scn_data_key_seq,RemoteWebDriver driver){
		    Properties prop;
			try {
	     	...
	     	LangDataSingleton.getInstanceOf("_ENV_"+this.suiteName).put("CURR_TCNAME","HOMEPAGECloseLightBox2");
	     	this.scn_exe_tc_seq=scn_exe_tc_seq;
	     	this.testCaseId = "HOMEPAGE_CloseLightBox2";		
	}
		
	    
		
	public  synchronized void  executeTestcase() {
		catsLogger.console("Test case is running : HOMEPAGECloseLightBox2");
		String locString="";
		try{
		...
		if(setComputedActuals("5","","IFRAMES", "Home", "CloseXone","","") &&				
		executeTestStep("113",  "WAITTILLELEMENTDISPLAY","", suiteName, "HOMEPAGECloseLightBox2", 
		...
		executeTestStep("114",  "CLICK",": Click on <CloseXone>",	suiteName, "HOMEPAGECloseLightBox2", 
		...
		executeTestStep("115",  "WAIT",": Wait", suiteName, "HOMEPAGECloseLightBox2", 
			actualData,expectedResult, actualDataRef,expectedResultRef,"",	"", "",orRefName,"END IF","");
		//Control Flow IF block ENDS
		}
		arg.updateSkippedTestStepsForTestcase("HOMEPAGE_CloseLightBox2",executedStepIds,this.exnInstId);	
	}
