...
*
**/
public class  HEADERCLICKSIGNIN extends DynamicInstTestcaseClassExtend {
	    
    public HEADERCLICKSIGNIN(ITestContext context,long scn_exe_tc_seq,String scenarioId,long scn_data_key_seq,RemoteWebDriver driver){
		    Properties prop;
			try {
	     	...
	     	LangDataSingleton.getInstanceOf("_ENV_"+this.suiteName).put("CURR_TCNAME","HEADERCLICKSIGNIN");
	     	this.scn_exe_tc_seq=scn_exe_tc_seq;
	     	this.testCaseId = "HEADER_CLICKSIGNIN";		
	}
		
	    
		
	public  synchronized void  executeTestcase() {
		catsLogger.console("Test case is running : HEADERCLICKSIGNIN");
		String locString="";
		try{
		...
		//Test step 1.
		setComputedActuals("5","","Custom", "","SignInButton","","");			
		executeTestStep("116",  "EXPLICITWAIT",": Explicitly Wait for <5>",	suiteName, "HEADERCLICKSIGNIN", 
			actualData,expectedResult,actualDataRef,expectedResultRef,"",	"", "",orRefName,"","");
				
		//Test step 2.
		setComputedActuals("","","Custom", "TopMenu","SignInButton","$Header.SignInLink.FIELD1","");			
		executeTestStep("117",  "CLICKLINK",": Click Link on <$Header.SignInLink.FIELD1> present on the page",	suiteName, "HEADERCLICKSIGNIN", 
			actualData,expectedResult,actualDataRef,expectedResultRef,"",	"", "",orRefName,"","");
				
		//Test step 3.
		setComputedActuals("3","","Custom", "TopMenu","","","");			
		executeTestStep("118",  "WAIT",": Wait",	suiteName, "HEADERCLICKSIGNIN", 
			actualData,expectedResult,actualDataRef,expectedResultRef,"",	"", "",orRefName,"","");
		arg.updateSkippedTestStepsForTestcase("HEADER_CLICKSIGNIN",executedStepIds,this.exnInstId);	
	}
