...
*
**/
public class  COMMONCloseWindow extends DynamicInstTestcaseClassExtend {
	    
    public COMMONCloseWindow(ITestContext context,long scn_exe_tc_seq,String scenarioId,long scn_data_key_seq,RemoteWebDriver driver){
		    Properties prop;
			try {
	     	...
	     	LangDataSingleton.getInstanceOf("_ENV_"+this.suiteName).put("CURR_TCNAME","COMMONCloseWindow");
	     	this.scn_exe_tc_seq=scn_exe_tc_seq;
	     	this.testCaseId = "COMMON_CloseWindow";		
	}
		
	    
		
	public  synchronized void  executeTestcase() {
		catsLogger.console("Test case is running : COMMONCloseWindow");
		String locString="";
		try{
		...
		//Test step 1.
		setComputedActuals("","","CUSTOM", "","","","");			
		executeTestStep("218",  "Quit",": Quit the browser",	suiteName, "COMMONCloseWindow", 
			actualData,expectedResult,actualDataRef,expectedResultRef,"",	"", "",orRefName,"","");
		arg.updateSkippedTestStepsForTestcase("COMMON_CloseWindow",executedStepIds,this.exnInstId);	
	}
